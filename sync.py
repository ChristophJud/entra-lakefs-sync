...
USER_MATCH = os.environ.get('USER_FILTER', '*m365*')
...
class EntraID:
    ...
    def get_user_names(self, key='mail') -> List[str]:
        names = self._lookup('/users')
        return [name.get(key) for name in names if name.get("officeLocation") == "Basel Office"]
...
def sync_users(entra: EntraID, lakefs: LakeFSAuth, user_filter: Optional[str] = None,
               default_policies: Optional[List[str]] = None, dry_run=True):
    filtered_users = entra.get_user_names()
    if user_filter:
        filtered_users = [item for item in filtered_users if not fnmatch.fnmatch(item, user_filter)]
    print(f'found {len(filtered_users)} matching users')
    for user in filtered_users:
        print(user)


def sync_groups(entra: EntraID, lakefs: LakeFSAuth, group_filter: Optional[str] = None,
                default_policies: Optional[List[str]] = None, dry_run=True):
...
if __name__ == '__main__':
    ...
    sync_users(entra, lakefs, USER_MATCH, default_policies=policies, dry_run=DRY_RUN)
